...
@Component({
  selector: 'edittaco',
  templateUrl: 'edittaco.html'
})
export class EditTaco {
  currentTaco;
  tacoIndex;
  tortillaChoice = [
    {
  ...
  setTacoOptions() {
    this.costOfOneTaco = this.currentTaco.cost;
    this.numberOfTacos = this.currentTaco.quantity;
    this.tfRiceIsChecked = this.currentTaco.tortillaOptions.rice;
    this.tfBeansIsChecked = this.currentTaco.tortillaOptions.beans;

    for(let i = 0; i < this.tortillaChoice.length; i++) {
        if(this.tortillaChoice[i].displayName == this.currentTaco.tortilla) {
            this.tortillaChoice[i].isSelected = true;
        }
    }
    for(let i = 0; i < this.proteinChoice.length; i++) {
        for(let j = 0; j < this.currentTaco.protein.length; j++) {
            if(this.proteinChoice[i].displayName == this.currentTaco.protein[j]) {
                this.proteinChoice[i].isSelected = true;
            }
        }
    }
    for(let i = 0; i < this.toppingChoice.length; i++) {
        for(let j = 0; j < this.currentTaco.toppings.length; j++) {
            if(this.toppingChoice[i].name == this.currentTaco.toppings[j]) {
                this.toppingChoice[i].isSelected = true;
            }
        }
    }
    for(let i = 0; i < this.cheeseChoice.length; i++) {
        for(let j = 0; j < this.currentTaco.cheese.length; j++) {
            if(this.cheeseChoice[i].name == this.currentTaco.cheese[j]) {
                this.cheeseChoice[i].isSelected = true;
            }
        }
    }
    for(let i = 0; i < this.salsaChoice.length; i++) {
        for(let j = 0; j < this.currentTaco.salsa.length; j++) {
            if(this.salsaChoice[i].name == this.currentTaco.salsa[j]) {
                this.salsaChoice[i].isSelected = true;
            }
        }
    }
    for(let i = 0; i < this.sauceChoice.length; i++) {
        for(let j = 0; j < this.currentTaco.sauces.length; j++) {
            if(this.sauceChoice[i].name == this.currentTaco.sauces[j]) {
                this.sauceChoice[i].isSelected = true;
            }
        }
    }
  }
  ...
  submitEdit() {
    ...
    this.currentTaco = new Taco(tortillaName, tortillaOptions, selectedProteinsNames,
                        selectedToppingsNames, selectedCheeseNames, selectedSalsaNames,
                        selectedSauceNames, this.numberOfTacos, this.costOfOneTaco);
    this.tacoService.updateTaco(this.currentTaco, this.tacoIndex);
  }
  ...
  constructor(params: NavParams, public viewCtrl: ViewController, private tacoService: TacoService,
      ...
      this.tacoIndex = params.data.index;
      this.currentTaco = tacoService.getTaco(this.tacoIndex);
      this.setTacoOptions();
  }
}